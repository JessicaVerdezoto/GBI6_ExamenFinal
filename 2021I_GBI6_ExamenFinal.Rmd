...
#La tabla de datos indica las observaciones y el nivel de expresion en los genes 
#determinados para cada tipo de cancer analizado, como por ejemplo los tres 
#principales donde se observa que existen muchos datos para el cancer de Mama 
#BRCA y su alto nivel de expresion en el gen ESR1, mientras que para el 
#Cohorte Pan-Riñon KIPAN se muestra gran cantidad de vistas y su nivel de 
#expresion en el gen PTEN, para el Carcinoma Renal KIRP se observa poca 
#observancion y nivel de expresion en el gen en PTEN. 
```
...
tcga_boxplots=function(df1){
  ...
  img=df1[,c(1,2)]
  names(img)=c("fac1","ind1")
  nom=as.character(levels(img[,1]))
n=NULL

  for (i in 1:length(levels(img$fac1))){
   
    j=ggplot(img%>%filter(fac1==nom[i]),aes(fac1,ind1))+geom_jitter()+geom_boxplot(alpha=0.3)+xlab("Tipo de Cancer")+ylab("gen")+
...
cancertype_boxplots[]
ggsave("C:/Users/INTEL/Documents/GitHub/GBI6_ExamenFinal/results/boxplot3.png", width = 6, height = 8, dpi = 300)
```
...
reg_gen_expression=function(df){
  ...
  for (i in 1:ncol(carp)) {
    nb=NULL
      x=NULL
      y=NULL
      
      nb=c(carp[,i][1],carp[,i][2])
      
      
  nom=as.character(levels(df[,1]))
  
  x=df[which(df$gen==nom[nb[1]]),][,2]
  y=df[which(df$gen==nom[nb[2]]),][,2]
  dft=data.frame(x,y)
m=ggplot(data=dft,aes(x,y))+geom_point()+xlab(nom[nb[1]])+ylab(nom[nb[2]])
...
#El boxplot, diagrama de caja, sirve para sustentar la distribucion de los
#datos analizados para tipos de cancer, las expresiones de genes no
#resultaron con distribucion normal, es decir que los datos se encuentran 
#de manera asimétrica con respecto al valor central. 
#Por otro lado se osberva que los valores de expresion de los genes "PTEN" 
#y "GAPDH", estan concentrados en un solo lugar, pero se visualiza pocos 
#valores lejanos.
```
